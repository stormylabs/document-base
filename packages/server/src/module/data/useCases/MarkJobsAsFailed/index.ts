...
export default class MarkJobsAsFailedUseCase {
  private readonly logger = new Logger(MarkJobsAsFailedUseCase.name);
  constructor(
  ...
  public async exec(): Promise<Response> {
    try {
      ...
      for (const job of crawlJobs) {
        await this.crawlJobService.updateStatus(job._id, JobStatus.Failed);
      }

      const docIndexJobs = [...runningDocIndexJobs, ...pendingDocIndexJobs];

      for (const job of docIndexJobs) {
        await this.docIndexJobService.updateStatus(job._id, JobStatus.Failed);
      }

      this.logger.log(`Marked crawl jobs ${crawlJobs.length} jobs as failed`);
      this.logger.log(
        `Marked doc index jobs ${docIndexJobs.length} jobs as failed`,
      );
      return right(Result.ok());
    } catch (err) {
      console.log(err);
